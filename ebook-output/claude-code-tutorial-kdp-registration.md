# KDP Registration Package: Claude Code 完全実践ガイド

- Generated: 2026-07-05 10:48:03
- Project: claude-code-tutorial
- Metadata: C:\Dev\tutorials\claude-code-tutorial\.\.github\skills-config\ebook-build\claude-code-tutorial.metadata.yaml
- KDP Metadata: C:\Dev\tutorials\claude-code-tutorial\.\.github\skills-config\ebook-build\claude-code-tutorial.kdp.yaml

## 1. Bibliographic Information

| Field | Value |
|---|---|
| Title | Claude Code 完全実践ガイド |
| Title Pronunciation | — |
| Romanized Title | — |
| Subtitle | スキル・フック・メモリ・MCP を使いこなす |
| Subtitle Pronunciation | — |
| Romanized Subtitle | — |
| Author | 杜 文吉 |
| Language | ja-JP |
| Publisher | Duwenji Lab |
| Rights | Copyright (c) 杜 文吉 |
| Identifier | claude-code-tutorial |
| Publication Date | 2026-06-14 |
| ISBN | Typically not required for Kindle only |

## 2. Product Description

> Anthropic の AI コーディングアシスタント Claude Code を基礎から応用まで体系的に学べる実践教材。 スキルシステム・フック自動化・メモリ管理・MCP/エージェント連携を最小手順で習得できます。

## 3. Keywords (up to 7)

- Claude Code
- AI コーディング
- Anthropic
- 開発効率化
- スキル
- フック
- MCP

## 4. Categories (up to 3)

- Computers / Software Development
- Computers / Programming

## 5. Pricing and Distribution

| Field | Value |
|---|---|
| List Price | 1490 JPY |
| Royalty | 70% |
| Territories | Worldwide |
| KDP Select | No |

## 6. Upload Files

- EPUB: C:\Dev\tutorials\claude-code-tutorial\.\ebook-output\claude-code-tutorial.epub
- PDF: C:\Dev\tutorials\claude-code-tutorial\.\ebook-output\claude-code-tutorial.pdf
